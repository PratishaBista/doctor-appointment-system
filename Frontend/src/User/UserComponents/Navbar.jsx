  ...
  return (
    <div className="bg-white w-full shadow-md">
      <div className="max-w-[2200px] mx-auto flex justify-between items-center px-6 py-4">
        ...
        <img onClick={()=>setNotificationDropDown(!notificationDropDown)} className="h-[60px] mr-4" src="https://static.vecteezy.com/system/resources/previews/006/086/198/original/notification-icon-for-web-vector.jpg" alt="" />
              <p className=" absolute left-[33px] bottom-[24px] bg-red-500 rounded-full px-1 font-bold text-[16px]">
                  {notifications.length} 
                </p>
                {notificationDropDown ? (
  <div className="absolute top-[80px] left-[10px] h-[90vh] w-[18rem] bg-white z-50 border border-gray-200 shadow-xl rounded-xl overflow-y-auto">
    <h3 className="text-lg font-semibold text-gray-700 p-4 border-b">Notifications</h3>
    
    {/* Example Notification Items will cover after the model is ready */}
    {notifications.map((not, index) => (
        <div
          key={index}
          className="bg-blue-100 hover:bg-green-200 transition p-4 m-2 rounded-lg shadow-sm cursor-pointer"
