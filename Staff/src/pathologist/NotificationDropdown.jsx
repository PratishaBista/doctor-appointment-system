...
const NotificationDropdown = () => {
  const [pathologist_token] = useState(localStorage.getItem('pathologist_token') || '');
  const backendUrl = import.meta.env.VITE_BACKEND_URL;
  const [isOpen, setIsOpen] = useState(false);
  const [notifications, setNotifications] = useState([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [isLoading, setIsLoading] = useState(false);
  const { pathologistData } = useContext(PathologistContext)

  const fetchNotifications = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.get(`${backendUrl}/api/notifications/pathologist`, {
        headers: { pathologist_token },
        params: {
          userId: pathologistData?._id,
  ...
  const markAsRead = async (notificationId) => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/notifications/pathologist/mark-read`,
        { notificationId },
        { headers: { pathologist_token } }
      );
  ...
  const markAllAsRead = async () => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/notifications/pathologist/mark-all-read`,
        {},
        { headers: { pathologist_token } }
      );
  ...
  useEffect(() => {
    if (isOpen && pathologist_token) {
      fetchNotifications();
    }
  }, [isOpen, pathologist_token]);
