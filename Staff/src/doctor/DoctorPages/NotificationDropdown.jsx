...
const NotificationDropdown = () => {
  const { doctor_token, doctorData, backendUrl } = useContext(DoctorContext);
  const [isOpen, setIsOpen] = useState(false);
  const [notifications, setNotifications] = useState([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [isLoading, setIsLoading] = useState(false);

  console.log("NotificationDropdown is rendering");

  const fetchNotifications = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.get(`${backendUrl}/api/notifications/doctor`, {
  ...
  const markAsRead = async (notificationId) => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/notifications/doctor/mark-read`,
        { notificationId },
        { headers: { doctor_token } }
      );
  ...
  useEffect(() => {
    if (doctor_token && doctorData?._id) {
      fetchNotifications();
    }
  }, [doctor_token, doctorData]);


  return (
    <div className="relative z-[9001]">
      <button
